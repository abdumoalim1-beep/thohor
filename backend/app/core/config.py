...
class Settings(BaseSettings):
    ...
    # Google (SerpAPI) costs real money per call, so it gets a smaller
    # budget — 20 is still comfortably above MIN_SUCCESSFUL_QUERIES_FOR_
    # MEASURED (10, see app.preview_reports.visibility) so google_score
    # stays a real percentage, not a guess. The AI leg (OpenAI web_search)
    # is cheap enough to run against a larger query set on top of these
